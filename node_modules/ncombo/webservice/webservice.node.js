...
var WebServiceClient = function(host, port, secure, wsEndpoint) {
	...
	self.disconnect = function() {
		if(self._wsSocket) {
			self._wsSocket.removeListener('disconnect', self.disconnect);
			self._wsSocket.disconnect();
		}
		
		if(self._connecting) {
			var i;
			for(i in self._connectCallbackMap) {
				clearTimeout(self._connectCallbackMap[i].timeout);
				self._connectCallbackMap[i].callback(self._connectFailedMessage);
			}
			self._connectCallbackMap = {};
		}
	...
	self._connectCallback = function(err) {
		var i;
		for(i in self._connectCallbackMap) {
			self._connectCallbackMap[i].callback(err);
			clearTimeout(self._connectCallbackMap[i].timeout);
			delete self._connectCallbackMap[i];
		}
	}
		
	self._connectHandler = function() {
		self.removeListener('connection', self._connectHandler);
	...
	self._init = function() {
		self._wsSocket.removeListener('disconnect', self._connect);
	...
	self._connect = function() {
		...
		if(connected) {
			...
			self._wsSocket.removeListener('connect', self._init);
			self._wsSocket.on('connect', self._init);
		}
		
		self._wsSocket.removeListener('disconnect', self.disconnect);
		self._wsSocket.on('disconnect', self.disconnect);
	}
	...
	self.connect = function(callback) {
		...
		if(callback) {
			var connectTimeout = setTimeout(function() {
				delete self._connectCallbackMap[id];
				self.disconnect();
				callback(self._connectFailedMessage);
			}, self._timeout);
			
			self._connectCallbackMap[id] = {callback: callback, timeout: connectTimeout};
		}
		...
		if(self._connected) {
			...
			self.removeListener('connection', self._connectHandler);
			self.on('connection', self._connectHandler);
		} else if(self._wsSocket) {
			if(self._wsSocket.socket.connected) {
				self._wsSocket.once('disconnect', self._connect);
				self.disconnect();
			} else {
				self._connect();
			}
...
var WebService = function() {
	...
	self.watch = function(siReq) {
		...
		self._watchMap[id].push({ns: request.ns, event: request.event, client: client, handler: handler});
