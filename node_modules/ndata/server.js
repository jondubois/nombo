...
var FlexiMap = function(object) {
	...
	self._isObject = function(object) {
	...
	if(object) {
		var i;
		if(object instanceof Array || self._isObject(object)) {
			self._data = object;
		}
	}
	...
	self._set = function(keyChain, value) {
		var key = keyChain[0];
		if(keyChain.length < 2) {
			if(!(value instanceof FlexiMap) && (self._isObject(value) || (value instanceof Array))) {
...
var actions = {
	...
	emit: function(command, socket) {
		if(watchMap[command.event]) {
			...
			send(socket, {id: command.id, type: 'response', action: 'emit', value: command.value, event: command.event});
		}
	}
}
...
server.on('connection', function(sock) {
	...
	sock.on('data', function(commandBuffer) {
		var commands = formatter.parse(commandBuffer);
		var i;
		for(i in commands) {		
