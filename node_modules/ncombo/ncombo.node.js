...
var nCombo = new (function() {
	...
	self._getAppDef = function(frameworkURL, req) {
		...
		appDef.releaseMode = self._releaseMode;
		appDef.timeout = self._timeout;
	...
	self._getLoaderCode = function(req) {
		...
		var loaderCode = '$loader.init("' + frameworkURL + '","' + routToScriptURL + '","' +
				loadScriptURL + '",' + resString + ',' + appString + ',' + (self._releaseMode ? 'false' : 'true') + ');';
	...
	self._sessionHandler = function(req, res, next) {
		...
		if(req.url == '/~startscript') {
			...
			if(url == self._rootTemplateURL) {
				...
				if(url == '/node_modules/ncombo/core/session.js' || url == '/node_modules/socket.io-client/dist/socket.io.min.js' || url == '/node_modules/ncombo/loader.js' ||
						url == '/node_modules/ncombo/styles/ncombo.css') {
					
					// never cache session.js
					if(!skipCache && self._releaseMode && cache.hasFile(filePath)) {
						...
						fs.readFile(filePath, function(err, data) {
							if(err) {
								...
								var js = template({endpoint: self._wsEndpoint, cacheVersion: self._launchTime, port: self._port});
								
								if(!skipCache && self._releaseMode) {
	...
	self._setFileResponseHeaders = function(res, filePath, mimeType, forceRefresh) {	
		...
		if(self._releaseMode && !forceRefresh) {
			var now = new Date();
	...
	self._cleanupWorker = function(worker) {
		var pid = worker.process.pid;
		self._dataClient.get('__workersessions.' + pid, function(err, sessionIDs) {
			if(err) {
				console.log('   nCombo Error - Failed to get worker sessions');
			} else {
				var i;
				for(i in sessionIDs) {
					self._dataClient.remove('__opensockets.' + i);
				}
			}
		});
		
		self._dataClient.remove('__workersessions.' + pid, function(err) {
			if(err) {
	...
	self._getSession = function(id, callback) {
		if(!self._dataClient) {
			return null;
		}
		
		self._dataClient.get('__opensessions');
	}
	
	self.start = function(port, options) {
		self._port = parseInt(port);
		...
		if(options.workers) {
			self._workerCount = options.workers;
		} else {
			self._workerCount = numCPUs;
		}
		...
		var begin = function() {
			...
			self._wsSocks.on('connection', function(socket) {
				...
				self._dataClient.set('__workersessions.' + cluster.worker.process.pid + '.' + socket.id, 1);
				...
				socket.on('localCall', function(request) {
					var origin = {address: socket.handshake.address.address, port: self._port};
				...
				socket.on('disconnect', function() {
					...
					self._dataClient.remove('__workersessions.' + cluster.worker.process.pid + '.' + socket.id);
				});
			});
		...
		if(cluster.isMaster) {
			...
			portScanner.findAPortNotInUse(self._port + 1, self._port + 1000, 'localhost', function(error, datPort) {
				dataPort = datPort;
				...
				self._dataServer.on('ready', function() {
					...
					var handler = function(data) {
						if(data.action == 'init') {
							...
							if(activeProcesses >= self._workerCount) {
								console.log('   nCombo server started on port ' + self._port + ' - Number of workers: ' + activeProcesses);
							} else {
								activeProcesses++;
								worker = cluster.fork();
