...
var Global = function (dataClient, eventEmitter, namespace) {
	this._dataClient = dataClient;
...
Global.prototype.ns = function (namespace) {
	return new Global(this._dataClient, this._eventEmitter, namespace);
};


var Session = function (sessionId, dataClient, eventEmitter, namespace) {
	this.id = sessionId;
	this._dataClient = dataClient;
	this._eventEmitter = eventEmitter;
	this._namespace = namespace ? namespace : '__main';
...
Session.prototype.ns = function (namespace) {
	return new Session(this.id, this._dataClient, this._eventEmitter, namespace);
};
...
var IOClusterClient = module.exports.IOClusterClient = function (options) {
	...
	this._handshakeExpiry = options.handshakeExpiry;
...
IOClusterClient.prototype.handshake = function (handshakeData, callback) {
	var self = this;
	
	if(handshakeData.address == null || handshakeData.id == null) {
		callback && callback("Failed handshake - Invalid handshake data");
	} else {
		var ioSocket = this.socket(handshakeData.id);
		var remoteAddr = handshakeData.address;
		...
		var acceptHandshake = function () {
			async.waterfall([
				function (cb) {
					self._dataClient.expire(self._keyManager.getGlobalDataKey(['__meta', 'addresses', remoteAddr]), self._handshakeExpiry, cb);
				},
				function (cb) {
					self._dataClient.set(self._keyManager.getGlobalDataKey(['__meta', 'addresses', remoteAddr, 'sockets', handshakeData.id]), 1, cb);
				}
			], function (err) {
				if (err) {
					callback && callback(err);
				} else {
					ioSocket.expire(null, self._handshakeExpiry, function (err) {
						if (err) {
							callback && callback(err);
						} else {
							ioSocket.set(['__meta', 'handshaken'], handshakeData, callback);
						}
					});
				}
			});
		...
		if (this._addressSocketLimit > 0) {
			this.getAddressSockets(remoteAddr, function(err, sockets) {
				if (err) {
					callback && callback("Failed handshake");
				} else {
...
IOClusterClient.prototype.getHandshake = function (id, callback) {
	var ioSocket = this.socket(id);
	var handshake = null;
	ioSocket.get(['__meta', 'handshaken'], function (err, value) {
		if (!err) {
			handshake = value || null;
		}
		callback(err, handshake);
	});
};
...
IOClusterClient.prototype.bind = function (socket, callback) {
	...
	this._sessions[socket.ssid].sockets[socket.id] = socket;
	
	this._addresses[socket.address] = {
		dataKey: socket.addressDataKey,
		sockets: {}
	};
	this._addresses[socket.address].sockets[socket.id] = socket;
...
IOClusterClient.prototype.global = function () {
	return new Global(this._dataClient, this._globalEmitter);
};

IOClusterClient.prototype.session = function (sessionId) {
	return new Session(sessionId, this._dataClient, this._sessionEmitter);
};
...
IOClusterClient.prototype._handleSocketEvent = function (e) {
	...
	this._socketEmitter.emit(e.ns + '.' + e.event, e.data);
...
IOClusterClient.prototype._handleSessionEvent = function (e) {
	...
	this._sessionEmitter.emit(e.ns + '.' + e.event, e.data);
...
IOClusterClient.prototype._handleGlobalEvent = function (e) {
	...
	this._globalEmitter.emit(e.ns + '.' + e.event, e.data);
