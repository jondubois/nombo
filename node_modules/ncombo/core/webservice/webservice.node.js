...
var WebServiceClient = function(host, port, secure, wsEndpoint) {
	...
	self.trackCall = function(cid, handler) {
		var hdlr;
		...
		self._wsClient.connect(function(err) {
			if(err) {
				...
				if(!self._serverWatchMap[event].length < 2) {
					...
					self._wsClient.trackCall(cid, ackHandler);
			...
			if(err) {
				...
				if(!event) {
					...
					if(unwatchRequest.requests.length > 0) {
						self._wsClient.trackCall(cid, ackCallback);
...
var WebService = function() {
	...
	self.watch = function(siReq) {
		...
		client.ns(request.ns).watch(request.event, handler, siReq.end);
	}
