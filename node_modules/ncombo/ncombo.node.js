...
var nCombo = new (function() {
	...
	self._frameworkURL = '/node_modules/ncombo/';
	...
	self._parseSID = function(cookieString) {
		if(cookieString) {
			var result = cookieString.match(/(ncsid=)([0-9]*)/);
			if(result) {
				return result[2]
			}
		}
	...
	self._writeSessionStartScreen = function(req, res) {
		...
		var cacheKey = encoding + ':' + filePath;
		...
		if(self._options.release && cache.hasFile(cacheKey)) {
			self._router.respond(res, cacheKey, null, encoding);
		} else {
			var spinJSURL = '/node_modules/ncombo/libs/spin.js';
		
			var includeString = self._getScriptTag('/node_modules/ncombo/core/smartcachemanager.js', 'text/javascript') + "\n";
			includeString += self._getScriptTag(spinJSURL, 'text/javascript') + "\n";
			self.allowFullAuthResource(spinJSURL);
			...
			self._router.respond(res, cacheKey, html, encoding);
		}
	}
	...
	self._sessionHandler = function(req, res, next) {
		...
		if(req.url == '/~startscript') {
			...
			if(self._options.release && cache.hasFile(cacheKey)) {
				...
				self._router.respond(res, cacheKey, loaderCode, encoding);
			}
		} else {
			...
			if(url == self._rootTemplateURL) {
				...
				var skipCache = (url == '/node_modules/ncombo/core/smartcachemanager.js');
				
				if(skipCache || url == '/node_modules/socket.io-client/dist/socket.io.min.js' || url == '/node_modules/ncombo/core/session.js'
					...
					if(self._options.release && cache.hasFile(cacheKey)) {
						...
						fs.readFile(filePath, function(err, data) {
							if(err) {
								...
								if(url == '/node_modules/ncombo/core/smartcachemanager.js') {
									var template = handlebars.compile(data.toString());
									data = template({cacheVersion: '*/ = ' + self._launchTime + ' /*'});
								} else if(url == '/node_modules/ncombo/core/session.js') {
									var template = handlebars.compile(data.toString());
									data = template({endpoint: self._wsEndpoint, port: self._options.port});
								}
								
								self._router.respond(res, cacheKey, data, encoding);
							}
						});
					}
				} else {
	...
	self._setFileResponseHeaders = function(res, filePath, mimeType, forceRefresh) {	
		...
		if(self._options.release && !forceRefresh) {
			var now = new Date();
			var oneMonth = new Date(now.getTime() + self._options.cacheLife);
	...
	self._validateOptions = function(options, validationMap) {
		var i;
		for(i in options) {
			if(validationMap[i] && !validationMap[i](options[i])) {
				throw new Error("The specified '" + i + "' option value is invalid");
			}
		}
	}
	...
	self.start = function(options) {
		...
		var optionValidationMap = {
			port: function() {
				return isInt(arguments[0]);
			},
			release: function() {
				return (typeof arguments[0] == 'boolean');
			},
			protocol: function() {
				return (arguments[0] == 'http' || arguments[0] == 'https');
			},
			...
			cacheType: function() {
				return (arguments[0] == 'private' || arguments[0] == 'public');
			},
			origins: function() {
		...
		var appDef = self._getAppDef();
		self._options.minifyURLs = [appDef.appScriptsURL, appDef.frameworkURL + 'scripts/load.js', appDef.frameworkURL + 'ncombo-client.js', 
				appDef.frameworkURL + 'loader.js'];
		...
		if(self._options.protocol == 'http') {
			...
			if(self._options.protocolOptions) {
				self._server = https.createServer(self._options.protocolOptions, self._routStepper.run);
			} else {
		...
		var begin = function() {
			self._options.cacheVersion = self._launchTime;
			self._router.start(self._options);
			...
			self._wsSocks.on('connection', function(socket) {
				...
				socket.on('localCall', function(request) {
					var origin = {address: socket.handshake.address.address, port: self._options.port};
					var siReq = new ServerRequest(socket, request, session, self._global, origin, secure);
				...
				socket.on('remoteCall', function(request) {
					...
					var siReq = new ServerRequest(socket, request, session, self._global, origin, request.secure);
		...
		if(cluster.isMaster) {
					...
					var handler = function(data) {
						if(data.action == 'init') {
							...
							if(activeProcesses >= self._options.workers) {
								...
								activeProcesses++;
								worker = cluster.fork();
								worker.send({action: 'init', dataPort: dataPort, dataKey: pass, launchTime: self._launchTime});
			...
			var handler = function(data) {
				if(data.action == 'init') {
					...
					self._launchTime = data.launchTime;
					begin();
				}
			}
