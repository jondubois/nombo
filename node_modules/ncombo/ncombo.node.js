...
var NSEmitter = function(namespace, socket) {
	...
	self.emit = function(event, data) {
		if(gateway.hasWatcher(self.socket.id, self._namespace, event)) {
			var eventObject = {namespace: self._namespace, event: event, data: data};
			self.socket.emit('event', eventObject);
		}
	}
}
...
var Session = function(sessionID, socket) {
	...
	self.id = sessionID;
	self.socket = socket;
	self._mainNamespace = new NSEmitter('__main', self.socket);
	...
	self.ns = function(namespace) {
		if(!self._namespaces[namespace]) {
			self._namespaces[namespace] = new NSEmitter(namespace, self.socket);
		}
