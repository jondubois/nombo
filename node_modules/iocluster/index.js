...
var IOClusterClient = module.exports.IOClusterClient = function (options) {
	...
	this._mapper = this._defaultMapper;
	...
	for (var i in options.stores) {
		...
		(function (dataClient) {
			dataClient.on('ready', function () {
				...
				dataClient.expiryInterval = setInterval(function () {
					self._extendExpiries(dataClient);
				}, self._expiryReset);
			});
		})(dataClient);
	}
...
IOClusterClient.prototype._defaultMapper = function (keyChain) {
	var hash = 0;
	var i, j, str, len;
	...
	for (i in keyChain) {
		...
		len = str.length;
		for(j=0; j<len; j++) {
			hash = str.charCodeAt(j) + (hash << 6) + (hash << 16) - hash;
		}
	}
...
IOClusterClient.prototype.map = function (keyChain) {
	var index = this._mapper(keyChain) % this._dataClients.length;
	return this._dataClients[index];
};

IOClusterClient.prototype.destroy = function (callback) {
	for (var i in this._dataClients) {
		clearInterval(this._dataClients[i].expiryInterval);
...
IOClusterClient.prototype._processExpiryList = function (dataClient) {
	var self = this;
	var key;
	var keys = [];
	...
	if (keys.length > 0) {
		dataClient.expire(keys, this._dataExpiry, function() {
			self._processExpiryList(dataClient);
		});
	}
};

IOClusterClient.prototype._extendExpiries = function (dataClient) {
	...
	this._processExpiryList(dataClient);
};
