...
$n.RemoteNS = function(host, port, secure, wsEndpoint, namespace, wsSocket) {
	...
	self.watch = function(event, handler) {
		...
		if(self._serverWatchMap[event].length < 2) {
			self._wsSocket.emit('watchRemote', request);
		}
	}
	...
	self.unwatch = function(event, handler) {
		...
		if(!event) {
			...
			if(unwatchRequests.length > 0) {
				self._wsSocket.emit('unwatchRemote', unwatchRequests);
			}
		} else if(!handler) {
			if(self._serverWatchMap[event]) {
				unwatchRequests.push({
			...
			if(self._serverWatchMap[event]) {
				...
				if(unwatchRequests.length > 0 && self._serverWatchMap[event].length < 1) {
					self._wsSocket.emit('unwatchRemote', unwatchRequests);
				}
			}
		}
...
$n.remote = function(host, port, secure, wsEndpoint) {
	...
	if(!$n.remoteClientMap.hasOwnProperty(url)) {
		$n.remoteClientMap[url] = new (function(host, port, secure, wsEndpoint) {
			var self = this;
			self._namespaces = {};
			
			self.ns = function(namespace) {
				if(!self._namespaces[namespace]) {
					self._namespaces[namespace] = new $n.RemoteNS(host, port, secure, wsEndpoint, namespace, $n._wsSocket);
				}
			...
			self.exec = function() {
				...
				$n._wsSocket.emit('remoteCall', request);
			}
...
$n.local = new (function($n) {
	...
	self.ns = function(namespace) {
		if(!self._namespaces[namespace]) {
			self._namespaces[namespace] = new $n.NS(namespace, $n._wsSocket);
		}
		return self._namespaces[namespace];
	}
	...
	self.exec = function() {
		...
		$n._wsSocket.emit('localCall', request);
	}
