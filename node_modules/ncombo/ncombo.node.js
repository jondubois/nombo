...
var nCombo = new (function() {
	...
	self._sessionHandler = function(req, res, next) {
		...
		if(req.url == '/~startscript') {
			...
			if(url == self._rootTemplateURL) {
				...
				var skipCache = (url == '/node_modules/ncombo/core/smartcachemanager.js' || url == '/node_modules/ncombo/core/session.js');
				
				if(skipCache || url == '/node_modules/socket.io-client/dist/socket.io.min.js' || url == '/node_modules/ncombo/loader.js' ||
						url == '/node_modules/ncombo/styles/ncombo.css') {
					
					// never cache smartcachemanager.js or session.js
						...
						fs.readFile(filePath, function(err, data) {
							if(err) {
								...
								if(url == '/node_modules/ncombo/core/smartcachemanager.js') {
									var js = template({cacheVersion: '*/ = ' + self._launchTime + ' /*'});
								} else {
									var js = template({endpoint: self._wsEndpoint, port: self._options.port});
								}
	...
	self.start = function(options) {
		...
		var appDef = self._getAppDef();
		self._options.minifyDirURLs = [appDef.appScriptsURL];
