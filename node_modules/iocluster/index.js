...
var getSocketDataKey = function (socketId, key) {
	if(key == null) {
		...
		return '__iocl.sod.' + socketId + '.' + key;
	}
};
...
Global.prototype.broadcast = function (event, data, callback) {
	this._dataClient.broadcast(getGlobalEventKey(), {ns: this._namespace, event: event, data: data}, callback);
};

Global.prototype.emit = function (sessionId, event, data, callback) {
	this._dataClient.broadcast(getSessionEventKey(sessionId), {ns: this._namespace, session: sessionId, event: event, data: data}, callback);
};
...
Session.prototype._localizeDataKey = function (key) {
	return getSessionDataKey(this.id, key);
}
...
Session.prototype.emit = function (event, data, callback) {
	this._dataClient.broadcast(getSessionEventKey(this.id), {ns: this._namespace, session: this.id, event: event, data: data}, callback);
};

Session.prototype.countSockets = function (callback) {
	this._dataClient.count('__iocl.sessions.' + this._dataClient.escape(this.id), callback);
};

Session.prototype.destroy = function (callback) {
	this._dataClient.remove('__iocl.sessions.' + this._dataClient.escape(this.id), callback);
};
...
Socket.prototype._localizeDataKey = function (key) {
	return getSocketDataKey(this.id, key);
}

Socket.prototype.emit = function (event, data, callback) {
	this._dataClient.broadcast(getSocketEventKey(this.id), {ns: this._namespace, socket: this.id, event: event, data: data}, callback);
}
...
var IOClusterServer = module.exports.IOClusterServer = function (port, secretKey) {
	var self = this;
	this._dataServer = ndata.createServer(port, secretKey);
...
var IOClusterClient = module.exports.IOClusterClient = function (port, secretKey) {
	var self = this;
	
	this._dataClient = ndata.createClient(port, secretKey);
	
	this._dataClient.on('ready', function () {
		self._dataClient.watchOnce(getGlobalEventKey(), function () { self._handleGlobalEvent.apply(self, arguments); }, function(err) {
			if(err) {
				self.emit('fail', err);
			} else {
				self.emit('ready');
...
IOClusterClient.prototype.bind = function (socket, callback) {
	...
	async.parallel([
		function (cb) {
			self._dataClient.watchOnce(getSocketEventKey(socket.id), function () { self._handleSocketEvent.apply(self, arguments); }, cb);
		},
		function (cb) {
			self._dataClient.watchOnce(getSessionEventKey(socket.ssid), function () { self._handleSessionEvent.apply(self, arguments); }, cb);
		},
		function (cb) {
			self._dataClient.set('__iocl.sessions.' + self._dataClient.escape(socket.ssid) + '.' + self._dataClient.escape(socket.id), 1, cb);
		},
		function (cb) {
			self._dataClient.set('__iocl.addresses.' + socket.address + '.' + self._dataClient.escape(socket.id), 1, cb);
		},
	],
	function(err) {
...
IOClusterClient.prototype.unbind = function (socket, callback) {
	...
	this._dataClient.unwatch(getSocketEventKey(socket.id), null, function (err) {
		if(err) {
			callback(err);
		} else {
			...
			if(isEmpty(self._sessions[socket.ssid])) {
				...
				async.waterfall([
					function (cb) {
						self._dataClient.unwatch(getSessionEventKey(socket.ssid), null, cb);
					},
					function (cb) {
						self._dataClient.remove('__iocl.sessions.' + self._dataClient.escape(socket.ssid) + '.' + self._dataClient.escape(socket.id), cb);
					},
					function (cb) {
							...
							if(!err && value < 1) {
								session.destroy(function (err) {
									...
									if(err) {
										...
										self._dataClient.remove('__iocl.addresses.' + socket.address, cb);
									}
								});
							} else {
								cb(err);
...
IOClusterClient.prototype.getAddressSockets = function (ipAddress, callback) {
	self._dataClient.get('__iocl.addresses.' + self._dataClient.escape(ipAddress), function (err, data) {
		var sockets = [];
		var i;
		for(i in data) {
			sockets.push(i);
