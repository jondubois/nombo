...
var actions = {
	...
	expire: function(command, socket) {
		expiryManager.expire(command.key, command.value);
		var response = {id: command.id, type: 'response', action: 'expire'};
		send(socket, response);
	},
	
	unexpire: function(command, socket) {
		expiryManager.unexpire(command.key);
		var response = {id: command.id, type: 'response', action: 'unexpire'};
		send(socket, response);
	},
	
	getExpiry: function(command, socket) {
		var response = {id: command.id, type: 'response', action: 'getExpiry', value: expiryManager.getExpiry(command.key)};
		send(socket, response);
	},
...
setInterval(function() {
	console.log('nData:', dataMap.getData());
}, 5000);