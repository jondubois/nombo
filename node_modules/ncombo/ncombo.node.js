...
var GlobalEmitter = function(namespace, sessionMap, dataClient) {
	...
	self.broadcast = function(event, data) {
		...
		dataClient.get('__opensockets', function(sockets) {
			var i;
			for(i in sockets) {
				dataClient.emit('session ' + i + ' ' + self._namespace + '.' + event, data);
			}
		});
	}
	
	self.emit = function(sessionID, event, data) {
		dataClient.emit('session ' + sessionID + ' ' + self._namespace + '.' + event, data);
	}
}
...
var nCombo = new (function() {
	...
	self._writeSessionStartScreen = function(req, res) {
		...
		if(self._cacheMode && cache.hasFile(filePath)) {
			...
			var includeString = self._getScriptTag('/node_modules/socket.io/node_modules/socket.io-client/dist/socket.io.min.js', 'text/javascript') + "\n";
	...
	self.start = function(port, options) {
		...
		var begin = function() {
			...
			self._dataClient = ndata.createClient(dataPort, 'password');
		...
		if(cluster.isMaster) {
			portScanner.findAPortNotInUse(port + 1, port + 1000, 'localhost', function(error, port) {
				dataPort = port;
				
				// TODO: Make password random and pass it to workers and use to instantiate clients
				self._dataServer = ndata.createServer(dataPort, 'password');
				self._dataServer.on('ready', function() {
					var i, worker;
					
					for(i=0; i<numCPUs; i++) {
						worker = cluster.fork();
						worker.send({action: 'init', dataPort: dataPort});
					}
				});
			});
		} else {
