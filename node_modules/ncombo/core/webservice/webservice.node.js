...
var WebServiceClient = function(host, port, secure, wsEndpoint) {
	...
	self._disconnect = function() {
		if(!self._connected) {
			console.log('   Could not connect to remote WebSocket. Check that WebServiceClient arguments are correct and that the target system matches the specified security parameter.');
		}
	...
	self._connect = function(callback, forceNew) {
	...
	self.connect = function(callback) {
		if(self._wsSocket && self._connecting) {
			self._wsSocket.removeAllListeners('disconnect');
			self._wsSocket.on('disconnect', function() {
				self._disconnect();
