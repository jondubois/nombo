	...
	self.start = function(port, options) {
		...
		self._wsSocks.on('connection', function(socket) {
			...
			socket.on('init', function() {
				self._sessionMap[socket.id] = new Session(socket.id);
				self._sessionMap[socket.id].setSocket(socket);
			});
			
			socket.on('sic', function(request) {
				var siReq = new ServerInterfaceRequest(socket, request, self._sessionMap[socket.id]);
...
var NSEmitter = function(namespace, socket) {
	var self = this;
	self._socket = socket;
	...
	self.emit = function(event, data) {
		if(gateway.hasWatcher(self._socket.id, self._namespace, event)) {
			var eventObject = {namespace: self._namespace, event: event, data: data};
...
var Session = function(sessionID) {
	...
	self.id = sessionID;
	self._socket = null;
	self._mainNamespace = new NSEmitter('__main');
	...
	self.setSocket = function(socket) {
		self._socket = socket;
		self._mainNamespace.setSocket(self._socket);
	}
	
	self.emit = function(event, data) {
		self._mainNamespace.emit(event, data);
	}
	
	self.broadcast = function(event, data) {
		self._mainNamespace.broadcast(event, data);
	}
	
	self.ns = function(namespace) {
		if(!self._namespaces[namespace]) {
			self._namespaces[namespace] = new NSEmitter(namespace, self._socket);
		}
...
var ServerInterfaceRequest = function(socket, request, session) {
	var self = this;
	...
	self._mainNamespace = new NSEmitter('__main', self._socket);
	self._namespaces = {'__main': self._mainNamespace};
