...
function Manager(server, options) {
  ...
  if (this.settings['io cluster']) {
    this.ioCluster = this.settings['io cluster'];
  }

  this.handshakeExpiry = this.settings['handshake expires'];
...
Manager.prototype.handleClient = function (data, req) {
  ...
  var proceed = function () {
    ...
    if (handshaken) {
      ...
      if (!this.connected[data.id]) {
        ...
        var ioSocket = self.ioCluster.socket(data.id);
        ioSocket.expire(null, this.handshakeExpiry, function (err) {
          if (!err) {
            ioSocket.set(['__meta', 'handshaken'], handshaken);
          }
        });
        ...
        for (var i in this.namespaces) {
          if (this.namespaces.hasOwnProperty(i)) {
            ...
            if (i === '') {
              this.namespaces[i].handlePacket(data.id, {
                type: 'connect'
              });
            }
          }
  ...
  if (handshaken) {
    ...
    ioSocket.get(['__meta', 'handshaken'], function (err, value) {
      ...
      self.handshaken[data.id] = handshaken;
      proceed.apply(self);
    });
  }
};
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      var id = self.generateId(),
        hs = [
          id, self.enabled('heartbeats') ? self.get('heartbeat timeout') || '' : '', self.get('close timeout') || '', self.transports(data).join(',')
        ].join(':');

      var tempHandshake = newData || handshakeData;
      ...
      var respondSuccess = function () {
        ...
        self.onHandshake(id, tempHandshake);
        self.store.publish('handshake', id, tempHandshake);

        res.end(hs);
        self.log.info('handshake authorized', id);
      };

      var respondFailure = function () {
        writeErr(500, 'handshake failed');
        self.log.info('handshake failed');
      };

      if (tempHandshake.query && tempHandshake.query.disconnect) {
        respondSuccess();
      } else {
        var ioSocket = self.ioCluster.socket(id);
        ioSocket.expire(null, self.handshakeExpiry, function (err) {
          if (err) {
            respondFailure();
          } else {
            ioSocket.set(['__meta', 'handshaken'], tempHandshake, function (err) {
              if (err) {
                respondFailure();
              } else {
                respondSuccess();
...
Manager.prototype.handshakeData = function (data) {
  var connection = data.request.connection,
    connectionAddress, date = new Date;
...
Manager.prototype.verifyOrigin = function (request) {
  ...
  if (origin) {
    try {
      ...
      var ok = ~origins.indexOf(parts.hostname + ':' + parts.port) || ~origins.indexOf(parts.hostname + ':*') || ~origins.indexOf('*:' + parts.port);
