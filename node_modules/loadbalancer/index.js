...
var LoadBalancer = function (options) {
	...
	this._server.proxy.on('proxyError', function (err) {
		self.emit('error', err);
	});
...
LoadBalancer.prototype._watchWorkerStatuses = function () {
	...
	for (i in this.workers) {
		(function (i) {
			port = self.workers[i].port;
			socket = new ComSocket();
			socket.connect(self.workers[i].statusPort, 'localhost');
			
			socket.on('message', function (nonceBuffer) {
				var nonce = nonceBuffer.toString('utf8');
				var authMessage = {
					type: 'auth',
					data: self.dataKey,
					nonce: nonce
				...
				var cipher = crypto.createCipher('aes192', self.dataKey);
				msg = cipher.update(msg, 'utf8', 'base64');
