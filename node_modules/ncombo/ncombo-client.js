...
var $n = {
	...
	init: function(appDefinition) {
		$n._appPath = appDefinition.appPath;
		...
		$n._wsSocket = io.connect($n._wsEndpoint);
		$n._callbacks['ready'] = [];
		$n._callbacks['fail'] = [];
	...
	grab: {
		...
		*/
		...
		_processScriptEmbedQueue: function() {
			var curTag;
			while($n.grab._scriptTagPendingQueue.length > 0) {
				curTag = $n.grab._scriptTagPendingQueue[0];
				if(curTag.ready) {
					...
					$n.grab._scriptTagPendingQueue.shift();
				} else {
					break;
				}
			}
		...
		_processLinkEmbedQueue: function() {
			var curTag;
			while($n.grab._linkTagPendingQueue.length > 0) {
				curTag = $n.grab._linkTagPendingQueue[0];
				if(curTag.ready) {
					...
					$n.grab._linkTagPendingQueue.shift();
				} else {
					break;
				}
			}
		...
		loadAndEmbedScript: function(url, successCallback, errorCallback) {
			...
			$n.grab._scriptTagPendingQueue.push(tagData);
			$n.grab._loadDeepResourceToCache(url, function() {
				tagData.ready = true;
				$n.grab._processScriptEmbedQueue();
			}, errorCallback);
		},
		
		loadAndEmbedCSS: function(url, successCallback, errorCallback) {
			var tagData = {url: url, successCallback: successCallback, ready: false}
			$n.grab._linkTagPendingQueue.push(tagData);
			$n.grab._loadDeepResourceToCache(url, function() {
				tagData.ready = true;
				$n.grab._processLinkEmbedQueue();
			}, errorCallback);
		},
...
$n.Template = $n.mixin(function() {
	...
	self.grab = function(name) {
		...
		if(self._extRegex.test(self._name)) {
			var url = $n._appTemplatesURL + name;
		} else {
			var url = $n._appTemplatesURL + name + '.handlebars';
		}
