...
IOClusterClient.prototype.unbind = function (socket, callback) {
	...
	async.waterfall([
		...
		function (cb) {
			self._dataClient.remove(socket.dataKey, cb);
		},
		function (cb) {
			...
			session.countSockets(function (err, value) {
				if (err || value >= 1) {
					...
					self._dataClient.remove(socket.sessionDataKey, function (err) {
						if (err) {
							cb(err);
						} else {
							self._dataClient.remove(self._keyManager.getGlobalDataKey(['__meta', 'addresses', socket.address]), function (err) {
								if (!err) {
