...
var Session = function(sessionID, socketManager, dataClient, retryTimeout) {
	...
	self._removeSocket = function(socket) {
		...
		var removeSessionSocket = function() {
			dataClient.remove('__opensessions.' + self.id + '.' + socket.id, removeSessionSocketCallback);
		}
	...
	self.get = function(key, callback) {
		dataClient.get('__sessiondata' + self.id + '.' + key, callback);
	}
	
	self.set = function(key, value, callback) {
		dataClient.set('__sessiondata' + self.id + '.' + key, value, callback);
	}
	
	self.remove = function(key, callback) {
		dataClient.remove('__sessiondata' + self.id + '.' + key, callback);
	}
	
	self._destroy = function() {
		var destroySessionCallback = function(err) {
			if(err) {
				setTimeout(destroySession, retryTimeout);
			}
		}
		
		var destroySession = function() {
			dataClient.remove('__sessiondata' + self.id, destroySessionCallback);
		}
	...
	self.broadcast = function(event, data) {
		...
		dataClient.get('__opensessions', function(err, sessions) {
			if(err) {
				...
				var i;
				for(i in sessions) {
					dataClient.broadcast('__sessiondata' + i + '.' + self._namespace + '.' + event, data);
				}
			}
		});
	}
	...
	self.emit = function(sessionID, event, data) {
		dataClient.get('__opensessions.' + sessionID, function(err, socks) {
			if(err) {
				...
				dataClient.broadcast('__sessiondata' + sessionID + '.' + self._namespace + '.' + event, data);
			}
		});
	}
}
...
var nCombo = new (function() {
	...
	self._sessionTimeoutMaxRand = 60000;
	...
	self.start = function(options) {
		...
		if(self._options.autoSession) {
			self._sessionTimeoutMaxRand = 0;
		}
		...
		var begin = function() {
			...
			self._dataClient.on('ready', function() {
				var nStore = new nDataStore({client: self._dataClient});
				...
				self._wsSocks.on('connection', function(socket) {
					...
					var cleanupSessionCallback = function(err, data) {
						if(err) {
							...
							if(numSockets <= 1) {
								...
								ws.unwatchAll(session);
								removeWorkerSession();
								session._destroy();
							}
							
							session._removeSocket(socket);
						}
					}
					
					var cleanupSession = function() {
						session.getSockets(cleanupSessionCallback);
					}
					...
					socket.on('disconnect', function() {
						...
						var rand = Math.round(self._sessionTimeoutMaxRand * Math.random());
						
						setTimeout(function() {
							var disconnectSessionCallback = function(err, data) {
								if(err) {
									setTimeout(disconnectSession, self._retryTimeout);
								}
							}
							var disconnectSession = function() {
								self._dataClient.run(jsQuery, disconnectSessionCallback);
							}
							disconnectSession();
							cleanupSession();
