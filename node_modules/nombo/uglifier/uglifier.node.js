...
Uglifier.prototype._launchUglifier = function () {
	...
	this._uglifier.on('message', function (m) {
		if (m.type == 'result') {
			...
			var id = m.id;
			if (self._pendingTasks[id] && !self._pendingTasks[id].noTimeout) {
				self._pendingTasks[id].timeout = setTimeout(function () {
					var err = new Error('Minification task timed out');
...
Uglifier.prototype.minifyJS = function (options, callback) {
	...
	if (options.noTimeout) {
		this._pendingTasks[id].noTimeout = options.noTimeout;
	}
	this._uglifier.send({
		type: 'minify',
		id: id,
