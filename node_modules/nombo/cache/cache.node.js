var Cache = new (function () {
	...
	self._maxEntrySize = 10000;
	self._serverCacheLifeMillis = 1000;
	
	self.init = function (options) {
		self._excludeRegex = options.excludeRegex;
		self._maxEntrySize = options.maxEntrySize;
		self._serverCacheLifeMillis = options.serverCacheLifeMillis;
	};
	...
	self.set = function (encoding, key, data) {
		...
		if (self._cache.hasOwnProperty(fullKey)) {
			...
			var now = Date.now();
			if (now - lastCacheTime < self._serverCacheLifeMillis) {
				return false;
			}
