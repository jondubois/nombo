...
var NCOMBO_IS_FRESH = (ncCacheCookie && ncCacheCookie > 0) ? false : true;
...
var handshakeErrorHandler = function(err) {
	...
	if(err == 'handshake error') {
		...
		limitScript.src = smartCacheManager.setURLCacheVersion(NCOMBO_FRAMEWORK_URL + 'scripts/connectionlimit.js');
		head.appendChild(limitScript);
	}
}

var ncSetCookies = function(ssid) {
	if(!getCookie('ncssid')) {
		setCookie('ncssid', ssid);
	}
	setCookie('ncsoid', ssid);
}

var NCOMBO_SESSION_MANAGER = new (function() {
	var self = this;
	...
	self.startSession = function() {
		...
		var url = '{{endpoint}}';
		if(data) {
			url += '?data=' + io.JSON.stringify(data);
		}
		
		NCOMBO_SOCKET = io.connect(url);
		
		NCOMBO_SOCKET.socket.on('error', function(err) {
			callback(err);
		});
		
		NCOMBO_SOCKET.on('connect', function() {
			ncSetCookies(NCOMBO_SOCKET.socket.sessionid);
			if(callback) {
				callback(null, NCOMBO_SOCKET.socket.sessionid);
			}
		});
	}
})();
...
if(NCOMBO_AUTO_SESSION) {
	...
	NCOMBO_SOCKET.socket.on('error', handshakeErrorHandler);
