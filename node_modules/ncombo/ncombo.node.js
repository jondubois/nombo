...
var nCombo = new (function() {
			...
			if(url == self._rootTemplateURL) {
				...
				if(url == '/node_modules/ncombo/core/session.js' || url == '/node_modules/socket.io-client/dist/socket.io.min.js' || 
					...
					if(sid) {
						...
						self._dataClient.get(keyPath, function(data) {
							if(data) {
	...
	self._cleanupWorker = function(worker) {
		...
		self._dataClient.get('__workersessions.' + pid, function(sessionIDs) {
			var i;
			for(i in sessionIDs) {
		...
		self._dataClient.remove('__workersessions.' + pid);
	}
	...
	self.start = function(port, options) {
		...
		var begin = function() {
			...
			self._wsSocks.on('connection', function(socket) {
				...
				self._dataClient.set('__opensockets.' + socket.id, 1, function() {
					socket.emit('ready');
				});
