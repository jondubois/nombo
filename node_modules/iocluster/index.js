...
var IOCluster = module.exports.IOCluster = function (dataClient) {
	var self = this;
		...
		if(err) {
			...
			if(isEmpty(self._sessions[socket.ssid])) {
				...
				async.waterfall([
					...
					function (cb) {
						self._dataClient.count('__iocl.sessions.' + self._dataClient.escape(socket.ssid), function(err, value) {
							if(!err && value < 1) {
								self._dataClient.remove('__iocl.sessions.' + self._dataClient.escape(socket.ssid), cb);
							} else {
								cb(err);
							}
						});
