...
Manager.prototype.handleClient = function (data, req) {
  ...
  var proceed = function () {
    ...
    if (handshaken) {
      ...
      if (!this.connected[data.id]) {
		...
		self.ioCluster.handshake(handshaken);
  ...
  if (handshaken) {
    ...
    self.ioCluster.getHandshake(data.id, function (err, handshake) {
	  handshaken = handshake;
      self.handshaken[data.id] = handshake;
      proceed.apply(self);
    });
  }
};
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      if (tempHandshake.query && tempHandshake.query.disconnect) {
        ...
        self.ioCluster.handshake(tempHandshake, function (err) {
			if (err) {
				error(err);
			} else {
				respondSuccess();
