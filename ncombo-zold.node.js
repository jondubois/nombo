...
var nCombo = function() {
	...
	self._parseSSID = function(cookieString) {
		if(cookieString) {
	...
	self._writeSessionStartScreen = function(req, res) {
		...
		if(self._options.release && cache.has(encoding, req.url)) {
			...
			var includeString = self._createScriptTag(self._frameworkURL + 'smartcachemanager.js', 'text/javascript') + "\n\t";
			includeString += self._createScriptTag(self._timeCacheExternalURL, 'text/javascript') + "\n\t";
			includeString += self._createScriptTag(self._spinJSURL, 'text/javascript') + "\n\t";
			includeString += self._createScriptTag(self._frameworkSocketIOClientURL, 'text/javascript') + "\n\t";
			includeString += self._createScriptTag(self._appExternalURL + self._frameworkURL + 'session.js', 'text/javascript');
	...
	self._applyFileResponseHeaders = function(res, filePath, mimeType, forceRefresh) {
		if(!mimeType) {
	...
	self._createScriptCodeTag = function(code, type) {
		if(!type) {
	...
	self._createScriptTag = function(url, type) {
	...
	self._createStyleTag = function(url, type) {
		url = self._normalizeURL(url);
		if(self._options.release) {
			url = self._smartCacheManager.setURLCacheVersion(url);
		}
		return '<link rel="' + self._defaultStyleRel + '" type="' + type + '" href="' + url + '" />';
	}
		...
		var begin = function() {
			...
			self._ioClusterClient.on('ready', function() {
				...
				var handleHandshake = function(handshakeData, callback) {
					...
					handshakeData.ssid = self._parseSSID(handshakeData.headers.cookie);
