...
var Global = function (socketId, dataClientCluster, eventEmitter, namespace) {
	this.socketId = socketId;
	this._dataClientCluster = dataClientCluster;
	this._eventEmitter = eventEmitter;
	this._namespace = namespace ? namespace : '__';
	this._keyManager = new KeyManager();
	
	AbstractDataClient.call(this, this._dataClientCluster);
};
...
Global.prototype.broadcast = function (event, data, callback) {
	this._dataClientCluster.broadcast(this._keyManager.getGlobalEventKey(), {ns: this._namespace, event: event, data: data}, callback);
};
...
Global.prototype._emit = function (sessionId, event, data, excludeCurrentSocket, callback) {
	...
	this._dataClientCluster.broadcast(this._keyManager.getSessionEventKey(sessionId), eventData, callback);
};
...
Global.prototype.map = function (clientIndices) {
	this._dataClientCluster.map(clientIndices);
};

Global.prototype.getClientCount = function () {
	return this._dataClientCluster.getClientCount();
};

Global.prototype.getActiveClientCount = function () {
	return this._dataClientCluster.getActiveClientCount();
};

Global.prototype.ns = function (namespace) {
	return new Global(this.socketId, this._dataClientCluster, this._eventEmitter, namespace);
};
...
var IOClusterServer = module.exports.IOClusterServer = function (options) {
	...
	for (var i in options.stores) {
		...
		this._dataServers.push(dataServer);
...
var IOClusterClient = module.exports.IOClusterClient = function (options) {
	...
	this._allMapper = function (keyChain, count) {
		var output = [];
	...
	this._sessionMapper = function (keyChain) {
		return self.hash(keyChain[2] || '');
	};

	this._addressMapper = function (keyChain) {
		return self.hash(keyChain[4] || '');
	};
	
	for (var i in options.stores) {
		dataClient = ndata.createClient(options.stores[i].port, options.dataKey);
		this._dataClients.push(dataClient);
	}
...
IOClusterClient.prototype.hash = function (string) {
	var result = 0;
	
	var len = string.length;
	for (var i=0; i<len; i++) {
		result = string.charCodeAt(i) + (result << 6) + (result << 16) - result;
	}
	
	return result % this._dataClients.length;
};

IOClusterClient.prototype.setMapper = function (mapper) {
	this._mapper = mapper;
};

IOClusterClient.prototype.getMapper = function () {
	return this._mapper;
};

IOClusterClient.prototype.map = function (keyChain, mapper) {
	if (!mapper) {
		mapper = this._mapper;
	}
	
	var result = mapper(keyChain);
	if (typeof result == 'number') {
		return this._dataClients[result];
	} else if (result instanceof Array) {
		var dataClients = [];
		for (var i in result) {
			dataClients.push(this._dataClients[result[i]]);
		}
		return new ClientCluster(dataClients);
	}
...
IOClusterClient.prototype.destroy = function (callback) {
	...
	for (var i in this._dataClients) {
		this._dataClients[i].removeAll(callback);
	}
};
...
IOClusterClient.prototype._processExpiryList = function (expiryList, mapper) {
	var self = this;
	var key, hash, i;
	var keys = {};
	...
	for (i=0; i<this._expiryBatchSize; i++) {
		...
		if (key == null) {
			...
			hash = mapper(key, this._dataClients.length);
			if (keys[hash] == null) {
				keys[hash] = [];
			}
			keys[hash].push(key);
	...
	for (hash in keys) {
		if (keys[hash].length > 0) {
			this._dataClients[hash].expire(keys[hash], this._dataExpiry, function() {
				self._processExpiryList(expiryList, mapper);
			});
		}
	}
};
...
IOClusterClient.prototype._extendExpiries = function () {
	...
	this._processExpiryList(sessionExpiryList, this._sessionMapper);
	this._processExpiryList(addressExpiryList, this._addressMapper);
};
...
IOClusterClient.prototype._handshake = function (socket, callback) {
	...
	if (socket.address == null || socket.id == null) {
		...
		var acceptHandshake = function () {
			async.waterfall([
				function (cb) {
					socket.addressDataClient.expire([self._keyManager.getGlobalDataKey(['__meta', 'addresses', remoteAddr])], self._connectTimeout, cb);
				},
				function (cb) {
					socket.addressDataClient.set(self._keyManager.getGlobalDataKey(['__meta', 'addresses', remoteAddr, 'sockets', socket.id]), 1, cb);
				}
			], function (err) {
...
IOClusterClient.prototype.bind = function (socket, callback) {
	...
	socket.dataClient = this.map(socket.sessionDataKey, this._sessionMapper);
	socket.addressDataClient = this.map(socket.addressDataKey, this._addressMapper);
	...
	this._handshake(socket, function (err) {
		if (err) {
			...
			async.waterfall([
				function (cb) {
					socket.dataClient.watchOnce(socket.eventKey, function () { self._handleSocketEvent.apply(self, arguments); }, cb);
				},
				function (cb) {
					socket.dataClient.watchOnce(socket.sessionEventKey, function () { self._handleSessionEvent.apply(self, arguments); }, cb);
				},
				function (cb) {
					socket.dataClient.expire([socket.sessionDataKey], self._dataExpiry, cb);
				},
				function (cb) {
					socket.dataClient.expire([socket.dataKey], self._dataExpiry, cb);
				},
				function (cb) {
					socket.dataClient.set(self._keyManager.getSessionDataKey(socket.ssid, ['__meta', 'sockets', socket.id]), 1, cb);
				},
				function (cb) {
					socket.addressDataClient.expire([socket.addressDataKey], self._dataExpiry, cb);
				}
			],
			function(err) {
...
IOClusterClient.prototype.unbind = function (socket, callback) {
	...
	async.waterfall([
		function (cb) {
			socket.dataClient.unwatch(socket.eventKey, null, cb);
		},
		function (cb) {
			delete self._sockets[socket.id];
			if (self._addresses[socket.address]) {
				delete self._addresses[socket.address].sockets[socket.id];
				socket.addressDataClient.remove(self._keyManager.getGlobalDataKey(['__meta', 'addresses', socket.address, 'sockets', socket.id]));
			...
			if (self._sessions[socket.ssid] && isEmpty(self._sessions[socket.ssid].sockets)) {
				...
				socket.dataClient.unwatch(socket.sessionEventKey, null, cb);
			} else {
				cb();
			}
		}
...
IOClusterClient.prototype.getAddressSockets = function (ipAddress, callback) {
	...
	var dataClient = this.map(addressDataKey, this._addressMapper);
	dataClient.get(this._keyManager.getGlobalDataKey(addressDataKey), function (err, data) {
		var sockets = [];
		var i;
		for (i in data) {
			sockets.push(i);
...
IOClusterClient.prototype.global = function (socketId) {
	var clientCluster = new ClientCluster(this._dataClients);
	var global = new Global(socketId, clientCluster, this._globalEmitter);
	global.map(0);
	return global;
};

IOClusterClient.prototype.session = function (sessionId, socketId) {
	return new Session(sessionId, socketId, this._dataClients[this.hash(sessionId)], this._sessionEmitter);
};
...
IOClusterClient.prototype.socket = function (socketId, sessionId) {
	...
	return new Socket(socketId, this._dataClients[this.hash(sessionId)], this._socketEmitter);
};
