...
var $n = {
	...
	init: function(appDefinition) {
		...
		$n.ioSocket = NCOMBO_SOCKET;
		$n._cacheVersion = NCOMBO_CACHE_VERSION;
		
		$n.ioSocket.on('return', $n._callReturn);
		$n.ioSocket.on('event', $n._eventReceived);
...
$n.NS = function(namespace, wsSocket) {
	...
	self.watch = function(event, handler, ackCallback) {
		...
		if(self._serverWatchMap[event].length < 2) {
			...
			self.ioSocket.emit('watchLocal', request);
		} else {
			cb();
		}
	}
	...
	self.unwatch = function(event, handler, ackCallback) {
		...
		if(!event) {
			...
			if(unwatchRequest.requests.length > 0) {
				...
				self.ioSocket.emit('unwatchLocal', unwatchRequest);
			} else {
				cb();
			}
		} else if(!handler) {
...
$n.RemoteNS = function(host, port, secure, wsEndpoint, namespace, wsSocket) {
	...
	self.watch = function(event, handler, ackCallback) {
		...
		if(self._serverWatchMap[event].length < 2) {
			$n.trackRequest(cid, ackHandler);
			self.ioSocket.emit('watchRemote', request);
		} else {
			cb();
		}
	}
	...
	self.unwatch = function(event, handler, ackCallback) {
		...
		if(!event) {
			...
			if(unwatchRequest.requests.length > 0) {
				$n.trackRequest(cid, cb);
				self.ioSocket.emit('unwatchRemote', unwatchRequest);
			} else {
				cb()
			}
		} else if(!handler) {
...
$n.Template = $n.mixin(function() {
	...
	self.grab = function(name, templateDirURL) {
		...
		$n.grab._loadDeepResourceToCache(url, function(err, result) {
			if(err) {
		...
		$n._remoteClientMap[url] = new (function(host, port, secure, wsEndpoint) {
			...
			self.ns = function(namespace) {
				if(!self._namespaces[namespace]) {
					self._namespaces[namespace] = new $n.RemoteNS(host, port, secure, wsEndpoint, namespace, $n.ioSocket);
				}
			...
			self.exec = function() {
				...
				$n.ioSocket.emit('remoteCall', request);
			}
...
$n.local = new (function($n) {
	...
	self.ns = function(namespace) {
		if(!self._namespaces[namespace]) {
			self._namespaces[namespace] = new $n.NS(namespace, $n.ioSocket);
		}
		return self._namespaces[namespace];
	}
	...
	self.exec = function() {
		...
		$n.ioSocket.emit('localCall', request);
	}
...
$n.grab.loadedTemplates = new Object();
$n.grab.template = function(name, fromFrameworkDir) {
	if($n.res.hasTemplate(name)) {
