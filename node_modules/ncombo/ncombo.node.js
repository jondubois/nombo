...
var nCombo = new (function() {
	...
	self.start = function(options) {
		...
		self.allowFullAuthResource('/node_modules/ncombo/scripts/connectionlimit.js');
		...
		var begin = function() {
			...
			if(self._options.maxConnectionsPerAddress > 0) {
				self._io.set('authorization', function (handshakeData, callback) {
					...
					if(!self._connectedAddresses.hasOwnProperty(remoteAddress)) {
						self._connectedAddresses[remoteAddress] = 0;
					}
					if(self._connectedAddresses[remoteAddress] < self._options.maxConnectionsPerAddress) {
						self._connectedAddresses[remoteAddress]++;
						callback(null, true);
					} else {
						callback("reached connection limit for the address '" + remoteAddress + "'", false);
					}
				});
			}
			...
			self._wsSocks.on('connection', function(socket) {
				...
				socket.on('localCall', function(request) {
					var remoteAddress = {address: socket.handshake.address.address, port: self._options.port};
				...
				socket.on('remoteCall', function(request) {
					...
					var remoteAddress = {address: socket.handshake.address.address, port: portNum};
				...
				socket.on('disconnect', function() {
					...
					if(self._connectedAddresses.hasOwnProperty(remoteAddress)) {
						if(--self._connectedAddresses[remoteAddress] <= 0) {
							delete self._connectedAddresses[remoteAddress];
						}
					}
			...
			self._server.on('listening', function() {
				gateway.init(__dirname + '/../../app/serverinterfaces/', self._dataClient, self._privateExtensionRegex);
