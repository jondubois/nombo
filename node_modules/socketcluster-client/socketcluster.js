...
module.exports.connect = function (uri, options) {
	return new ClusterSocket(uri, options);
};
});
...
ClusterSocket.prototype._updateDest = function () {
	if (this.id != this._destId) {
		var matches = this.id.match(this._destRegex);
		if (matches) {
			this.hostname = matches[1] || this.hostname;
			this.port = parseInt(matches[2]) || this.port;
		}
		this._destId = this.id;
	}
};

ClusterSocket.prototype.createTransport = function (name) {
	this._updateDest();
