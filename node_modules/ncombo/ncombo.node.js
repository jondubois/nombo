...
var ServerInterfaceRequest = function(socket, request, session, global, remoteAddress) {
	var self = this;
	self.session = session;
	self.global = global;
	self.remoteAddress = remoteAddress.address;
	self.remotePort = remoteAddress.port;
	...
	self.start = function(port, options) {
		...
		self._wsSocks.on('connection', function(socket) {
			...
			socket.on('sic', function(request) {
				var siReq = new ServerInterfaceRequest(socket, request, self._sessionMap[socket.id], self._global, socket.handshake.address);
