...
var run = function (code, context) {
	...
	var dataContext;
	if (context) {
		dataContext = dataMap.get(context);
	} else {
		dataContext = dataMap;
	}
	
	return Function('return (' + escapeBackslashes(code) + ')(arguments[0], arguments[1], arguments[2]);')(dataContext, eventMap, expiryManager);
}
...
var actions = {
	...
	broadcast: function (command, socket) {
		var sockets = eventMap.get('socket');
		var i, sock, eventKey;//, eventString;
		for (i in sockets) {
			//eventString = 'socket.' + i + '.' + command.event;
			eventKey = ['socket', i, command.event];
