...
IOClusterClient.prototype.bind = function (socket, callback) {
	...
	async.parallel([
		function (cb) {
			self._dataClient.watchOnce(self._keyManager.getSocketEventKey(socket.id), function () { self._handleSocketEvent.apply(self, arguments); }, cb);
		},
		function (cb) {
			self._dataClient.watchOnce(self._keyManager.getSessionEventKey(socket.ssid), function () { self._handleSessionEvent.apply(self, arguments); }, cb);
		},
		function (cb) {
			self._dataClient.expire(self._keyManager.getSessionDataKey(socket.ssid), self._dataExpiry, cb);
		},
		function (cb) {
