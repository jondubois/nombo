...
KeyManager.prototype.getWorkerEventKey = function (workerPort) {
	return ['__iocl', 'see', String(workerPort)];
};

KeyManager.prototype.getSessionWorkerEventKey = function (sessionId) {
	var workerPort = sessionId.match(this._workerPortRegex);
	if (workerPort) {
		workerPort = workerPort[1];
	}
	return ['__iocl', 'see', workerPort];
};

KeyManager.prototype.getSocketWorkerEventKey = function (socketId) {
	var workerPort = socketId.match(this._workerPortRegex);
	if (workerPort) {
		workerPort = workerPort[1];
	}
	return ['__iocl', 'see', workerPort];
...
Global.prototype._emit = function (sessionId, event, data, excludeCurrentSocket, callback) {
	...
	this._privClientCluster.broadcast(this._keyManager.getSessionWorkerEventKey(sessionId), eventData, callback);
};
...
var Session = function (sessionId, socketId, dataClient, eventClient, eventEmitter, namespace) {
	this.id = sessionId;
	this.socketId = socketId;
	this._dataClient = dataClient;
	...
	this._namespace = namespace ? namespace : '__';
	this._keyManager = new KeyManager();
	this._workerEventKey = this._keyManager.getSessionWorkerEventKey(this.id);
};
...
Session.prototype.emit = function (event, data, callback) {
	this._eventClient.broadcast(this._workerEventKey, {ns: this._namespace, session: this.id, event: event, data: data}, callback);
};

Session.prototype.transmit = function (event, data, callback) {
	this._eventClient.broadcast(this._workerEventKey, {ns: this._namespace, session: this.id, event: event, data: data, exclude: this.socketId}, callback);
};
...
Session.prototype.ns = function (namespace) {
	return new Session(this.id, this.socketId, this._dataClient, this._eventClient, this._eventEmitter, namespace);
};


var Socket = function (socketId, dataClient, eventClient, eventEmitter, namespace) {
	this.id = socketId;
	this._dataClient = dataClient;
	this._eventClient = eventClient;
	this._eventEmitter = eventEmitter;
	this._namespace = namespace ? namespace : '__';
	this._keyManager = new KeyManager();
	this._workerEventKey = this._keyManager.getSocketWorkerEventKey(this.id);
};
...
Socket.prototype.emit = function (event, data, callback) {
	this._eventClient.broadcast(this._workerEventKey, {ns: this._namespace, socket: this.id, event: event, data: data}, callback);
};
...
Socket.prototype.ns = function (namespace) {
	return new Socket(this.id, this._dataClient, this._eventClient, this._eventEmitter, namespace);
};
...
var IOClusterClient = module.exports.IOClusterClient = function (options) {
	var self = this;
	this._workerPort = options.workerPort;
	...
	var randClientIndex = Math.floor(Math.random() * dataClients.length);
	dataClient = dataClients[randClientIndex];
	...
	var hasher = function (string) {
		...
		if (integer) {
			return integer % dataClients.length;
		}
		var result = 0;
	...
	this._globalEventKey = self._keyManager.getGlobalEventKey();
	this._workerEventKey = this._keyManager.getWorkerEventKey(this._workerPort);
	
	var allClientsReady = function () {
		async.parallel([
			function () {
				var cb = arguments[arguments.length - 1];				
				dataClient.watch(self._globalEventKey, self._handleGlobalEvent.bind(self), cb);
			},
			function () {
				var cb = arguments[arguments.length - 1];
				self._privClientCluster.watch(self._workerEventKey, self._handleTargetedEvent.bind(self), cb);
			}
		],
		function(err) {
			if (err) {
	...
	var clientNum = dataClients.length;
	for (var j in dataClients) {
		dataClients[j].on('ready', function () {
			if (++clientReadyCount >= clientNum) {
				allClientsReady();
			}
		});
	}
...
IOClusterClient.prototype.bind = function (socket, callback) {
	...
	socket.workerEventKey = this._workerEventKey;
...
IOClusterClient.prototype.session = function (sessionId, socketId) {
	var workerEventKey = this._keyManager.getSessionWorkerEventKey(sessionId);
	return new Session(sessionId, socketId, this._privClientCluster.map(sessionId)[0], this._privClientCluster.map(workerEventKey)[0], this._sessionEmitter);
};
...
IOClusterClient.prototype.socket = function (socketId, sessionId) {
	...
	var workerEventKey = this._keyManager.getSocketWorkerEventKey(socketId);
	return new Socket(socketId, this._privClientCluster.map(sessionId)[0], this._privClientCluster.map(workerEventKey)[0], this._socketEmitter);
};

IOClusterClient.prototype._handleTargetedEvent = function (e) {
