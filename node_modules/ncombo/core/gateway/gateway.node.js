...
var Gateway = new (function() {
	...
	self._call = function(siReq) {
		var serverInterface = siReq.getServerInterface();
		var method = siReq.getMethod();
		var data = siReq.getData();
		
		if(self.isAllowed(serverInterface, method)) {
			self._serverInterfaces[serverInterface][method](siReq, data);
		} else {
			self._eventer.dispatchEvent('forbidden', siReq);
			throw "Exception: Unauthorized call to '" + method + "' method of '" + serverInterface + "' server interface";
		}
	}
	
	self.call = function(siReq) {
	...
	self.hasWatcher = function(socketID, namespace, event) {
		if(!self._serverWatchMap.hasOwnProperty(namespace)) {
			return false;
		}
		
		if(!self._serverWatchMap[namespace].hasOwnProperty(event)) {
			return false;
		}
		
		return self._serverWatchMap[namespace][event].hasOwnProperty(socketID);
	}
