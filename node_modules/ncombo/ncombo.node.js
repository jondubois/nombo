...
var nCombo = new (function() {
	...
	self._writeSessionStartScreen = function(req, res) {
		...
		if(self._options.release && cache.hasFile(cacheKey)) {
			...
			includeString += self._getScriptTag('/node_modules/ncombo/core/smartcachemanager.js', 'text/javascript') + "\n";
						...
						fs.readFile(filePath, function(err, data) {
							if(err) {
								...
								if(url == '/node_modules/ncombo/core/smartcachemanager.js') {
									js = template({cacheVersion: '*/ = ' + self._launchTime + ' /*'});
								} else {
									js = template({endpoint: self._wsEndpoint, port: self._options.port});
								}
								
								self._router.respond(res, cacheKey, js, encoding);
							}
						});
					}
				} else {
