var fs = require('fs'),
	...
	SmartCacheManager = require("ncombo/core/smartcachemanager").SmartCacheManager,
...
var Router = new (function() {
	...
	self.respond = function(res, cacheKey, data, encoding) {
	...
	self.rout = function(req, res) {
		...
		if(self._privateExtension && self._privateExtension.test(req.url)) {
			...
			if(self._releaseMode && cache.hasFile(cacheKey)) {
				...
				self.respond(res, cacheKey, null, encoding);
			} else if(self._releaseMode && cache.hasFile(filePath)) {
				self._setFileResponseHeaders(res, filePath);
				self.respond(res, cacheKey, cache.getFile(filePath), encoding);
			} else {
					...
					if(err) {
						...
						if(ext == '.less') {
							...
							less.render(data, function (e, css) {
								self.respond(res, cacheKey, css, encoding);
							});
						} else {
							if(ext == '.css') {
								data = self._versionDeepCSSURLs(data.toString());
							}
							self.respond(res, cacheKey, data, encoding);
						}
					}
				});
			}
