...
IOClusterClient.prototype._allMapper = function (keyChain, count) {
	var output = [];
...
IOClusterClient.prototype._sessionMapper = function (keyChain, count) {
	return this.hash(keyChain[2] || '') % count;
};

IOClusterClient.prototype._addressMapper = function (keyChain, count) {
	return this.hash(keyChain[4] || '') % count;
};
...
IOClusterClient.prototype.map = function (keyChain, mapper) {
	...
	var len = this._dataClients.length;
	var result = mapper(keyChain, len);
...
// TODO: Allow mapping global to any nData server
...
IOClusterClient.prototype.socket = function (socketId) {
	...
	return new Socket(socketId, this._dataClients[this.hash(socket.ssid)], this._socketEmitter);
};
