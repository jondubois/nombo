...
var nCombo = new (function() {
	...
	self._writeSessionStartScreen = function(req, res) {
		...
		if(self._options.release && cache.hasFile(filePath)) {
		...
		if(req.url == '/~startscript') {
			...
			if(url == self._rootTemplateURL) {
				...
				if(url == '/node_modules/ncombo/core/session.js' || url == '/node_modules/socket.io-client/dist/socket.io.min.js' || url == '/node_modules/ncombo/loader.js' ||
					...
					if(!skipCache && self._options.release && cache.hasFile(filePath)) {
						...
						fs.readFile(filePath, function(err, data) {
							if(err) {
								...
								var js = template({endpoint: self._wsEndpoint, cacheVersion: self._launchTime, port: self._options.port});
								
								if(!skipCache && self._options.release) {
