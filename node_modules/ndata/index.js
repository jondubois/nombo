...
var Client = function(port, host, secretKey, timeout) {
	...
	*/
	...
	self._stringifyQuery = function(query) {
	...
	self.registerDeathQuery = function(query, callback) {
		var queryString = self._stringifyQuery(query);
	...
	/*
		run(query,[ context,] callback)
	*/
	self.run = function() {
		var query = self._stringifyQuery(arguments[0]);
		var context = null;
		var callback;
		if(arguments[1] instanceof Function) {
			callback = arguments[1];
